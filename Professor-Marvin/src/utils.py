...
def getDataset(df, batch_size, cache_file=None, shuffle=True, parse_param=PARSE_PARAMS, scale=False):
    ...
    """
    print('pandas started to shuffle')
    #df = df.sample(frac=1)
    print('pandas shuffling over')
    data = tf.data.Dataset.from_tensor_slices((df["files"].tolist(), df["labels"].tolist()))
    # filename, labels = df["files"].tolist(), df["labels"].tolist()
    # data = pd.DataFrame(columns=["vector", "label"])
    # for i in range(128):
    #     if i%10==0: print('i: ', i)
    #     ls = [0 for _ in range(31)]
    #     datapoint = _parse_fn(filename[i], labels[i], parse_param, scale)
    #     ls[datapoint[1]] = 1
    #     data.loc[i] = [datapoint[0], np.array(ls, dtype=np.int64)]
    #print(data)




    # data = tf.data.Dataset.from_tensor_slices((data["vector"].tolist(), data["label"].tolist()))
    # print('elt spec: ', data.element_spec)
    ...
    print('shuffling begun')
    if shuffle:
        data = data.shuffle(buffer_size=df.shape[0])
    print('shuffling done')

    # for idx, e in enumerate(data.as_numpy_iterator(), start=1):
    #     print("INDEX: ", idx)
    #     if e[0].shape != (99,40):
    #         print(e[0].shape)
    #     if idx > 128:
    ...
    print("preMap: ", type(data))
    data = data.map(lambda k, v: (tf.ensure_shape(k, (None, 99,40)), tf.ensure_shape(v, (None,))))
    ...
    return data, steps #remember to fix this


def getDatasetNoMapTrain(df, batch_size, cache_file=None, shuffle=True, parse_param=PARSE_PARAMS, scale=False):
    """
    Creates a Tensorflow Dataset containing filterbanks, labels
    :param df: Dataframe with filenames and labels
    :param batch_size: Batch size of the input
    ...
    for i in range(2560):
        ...
        datapoint = parse_fn(filename[i], labels[i], parse_param, scale)
        vectors.append(datapoint[0])
        truth.append(datapoint[1])
    data = tf.data.Dataset.from_tensor_slices((vectors, truth))
    # if cache_file:
    #     #data = data.cache("../input/" + cache_file)
    #     data.cache()
...
def getDatasetNoMapVal(df, batch_size, cache_file=None, shuffle=True, parse_param=PARSE_PARAMS, scale=False):
    """
    Creates a Tensorflow Dataset containing filterbanks, labels
    :param df: Dataframe with filenames and labels
    :param batch_size: Batch size of the input
    ...
    :return: TF Dataset, Steps per epoch
    """
    print('pandas started to shuffle val')
    df = df.sample(frac=1)
    print('pandas shuffling over')
    filename, labels = df["files"].tolist(), df["labels"].tolist()
    vectors, truth = [], []
